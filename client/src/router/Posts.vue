<script setup>
    ...
    const fetchPosts = async () => {
        try {
            ...
            const response = await axios.get(`http://127.0.0.1:8000/trad_posts?page=${page.value}`);
...
<template>
        ...
        <DetailHeader img_url="/src/assets/pics/bg_1.jpg" header_title="طب سنتی"></DetailHeader>
                ...
                <div class="row">
                    <div class="col-lg-8 ftco-animate">
                        <div class="row">
                            ...
                            <div v-for="post in posts" :key="post._id" class="col-md-12 d-flex ftco-animate">
                                <div class="blog-entry align-self-stretch d-md-flex">
                                    <router-link :to="getHrefUrl(post._id)" class="block-20"
                                     :style="{ backgroundImage: `url('/src/assets/pics/${post.img}')` }">
                                    </router-link>
                                    <div class="text d-block pr-md-4">
                                        <div class="meta mb-3">
                                            <div><a href="#">{{ post.datetime }}</a></div>
                                            <div><a href="#">کاربر</a></div>
                                            <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                                        </div>
                                        <h3 class="heading"><a href="#">{{ post.title }}</a></h3>
                                        <p>{{ post.description }}</p>
                                        <p><router-link :to="getHrefUrl(post._id)" class="btn btn-primary py-2 px-3">بیشتر بخوانید</router-link>
                                        </p>
                                    </div>
                                </div>
                            </div>

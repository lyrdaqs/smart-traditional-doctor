<script setup>
    ...
    const fetchIllnes = async () => {
        try {
            const illnesId = route.params.id;
            const response = await axios.get(`http://127.0.0.1:8000/illness/${illnesId}`);
            illnes.value = response.data;
    ...
    const addComment = async () => {
        try {
            setToken()
            const illnesId = route.params.id; 
            await axios.post(`http://127.0.0.1:8000/illness/${illnesId}/comments/add`, {"content": comment.value});
            comment.value = ""
            await fetchIllnes()
        } catch (err) {
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-8 ftco-animate">
                        <h2 class="mb-3">{{ illnes.title }}</h2>
                        <p>
                            <img :src="getImageUrl(illnes.img_detail)" alt="" class="img-fluid">
                        </p>
                        <div v-html="htmlContent"></div>

                        <p v-for="img in illnes.img_detail_items">
                            <img :src="getImageUrl(img)" alt="" class="img-fluid">
                        </p>

                        <div class="tag-widget post-tag-container mb-5 mt-5">
                            <div class="tagcloud">
                                <router-link :to="getHrefTag(tag)" v-for="tag in illnes.tags" class="tag-cloud-link">{{ tag }}</router-link>
                            </div>
                        </div>
                        ...
                        <div class="pt-5 mt-5">
                            ...
                            <ul class="comment-list">
                                ...
                                <li class="comment" v-for="comment in illnes.comments">
                                    <div class="vcard bio">
